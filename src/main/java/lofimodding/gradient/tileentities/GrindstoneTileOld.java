...
public class GrindstoneTileOld extends ProcessorTileOld<GrindingRecipe, ManualEnergySource, GrinderProcessor> {
...
//TODO        return RecipeUtils.getRecipe(GrindingRecipe.TYPE, recipe -> recipe.matchesItems(this.temp, INPUT_SLOT, INPUT_SLOT)).isPresent();
      }

      return GrindstoneTileOld.this.forceInsert;
    }
    ...
    protected void onContentsChanged(final int slot) {
      if(slot == INPUT_SLOT) {
        ...
        if(!stack.isEmpty()) {
          if(!GrindstoneTileOld.this.hasRecipe()) {
            GrindstoneTileOld.this.updateRecipe();
          }
        } else {
          GrindstoneTileOld.this.clearRecipe();
        }
      }

      GrindstoneTileOld.this.sync();
    }
  };
  ...
  public GrindstoneTileOld() {
    super(GradientTileEntities.GRINDSTONE.get(), new ManualEnergySource(20, 1), new GrinderProcessor());
  }
...
//TODO    RecipeUtils.getRecipe(GrindingRecipe.TYPE, r -> r.matches(this.stages, this.inv, 0, 0)).ifPresent(this::setRecipe);
  }
