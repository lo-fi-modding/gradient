...
public class MechanicalMixingBasinRenderer extends TileEntityRenderer<MechanicalMixingBasinTile> {
  private IBakedModel rod;

  public MechanicalMixingBasinRenderer(final TileEntityRendererDispatcher rendererDispatcher) {
    super(rendererDispatcher);
  }

  @Override
  public void render(final MechanicalMixingBasinTile te, final float partialTicks, final MatrixStack matrixStack, final IRenderTypeBuffer buffer, final int combinedLight, final int combinedOverlay) {
  ...
  private void renderRod(final MechanicalMixingBasinTile te, final MatrixStack mat, final IRenderTypeBuffer renderer, final int combinedLight, final int combinedOverlay) {
    if(this.rod == null) {
      this.rod = Minecraft.getInstance().getModelManager().getModel(Gradient.loc("block/" + GradientIds.MECHANICAL_MIXING_BASIN + "_rod"));
    }
